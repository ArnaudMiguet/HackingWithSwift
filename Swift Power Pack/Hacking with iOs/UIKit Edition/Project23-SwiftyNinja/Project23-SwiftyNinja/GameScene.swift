...
class GameScene: SKScene {
    ...
    let bombeOnceEveryNTimes = 6
    let fuseOffset = CGPoint(x: 76, y: 64)
    let windowSideOffet = 64
    let enemySize = 64
    let speedFactor = 40
    ...
    func createEnemy(forceBomb: ForceBomb = .random) {
        ...
        var enemyType = Int.random(in: 0...bombeOnceEveryNTimes)
        ...
        let randomPosition = CGPoint(x: Int.random(in: windowSideOffet...Int(size.width) - windowSideOffet), y: -enemySize * 2)
        ...
        } else if randomPosition.x < size.width / 2 {
            randomXVelocity = Int.random(in: 3...5)
        } else if randomPosition.x < size.width / 4 * 3 {
            randomXVelocity = -Int.random(in: 3...5)
        } else {
            randomXVelocity = -Int.random(in: 8...15)
        }
        ...
        enemy.physicsBody?.velocity = CGVector(dx: randomXVelocity * speedFactor, dy: randomYVelocity * speedFactor)
